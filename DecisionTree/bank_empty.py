...
def find_common_value(test_data_m, feature_name):
    feature_value_list = test_data_m[feature_name].unique()
    
    common_value=""
    value_count=0
    for c in feature_value_list:
        if(c=='unknown'):
            continue
        if(value_count<functools.reduce(lambda sub, ele: sub * 10 + ele, test_data_m[test_data_m[feature_name] == c].shape)):
            value_count=functools.reduce(lambda sub, ele: sub * 10 + ele,test_data_m[test_data_m[feature_name] == c].shape)
            common_value=c

    return common_value

column_labels = ["age", "job", "marital", "education", "default", 
                 "balance", "housing", "loan", "contact", "day", 
                 "month", "duration", "campaign", "pdays", "previous", 
                 "poutcome", "y"]

label_attribute = ["yes","no"]

train_data_m = pd.read_csv("bank\\train - Copy.csv",names=column_labels, header=None) #importing the dataset from the disk
test_data_m = pd.read_csv("bank\\test.csv",names=column_labels, header=None) 
#change unknown data
for c in column_labels:
   common_value=  find_common_value(train_data_m,c)
   train_data_m=train_data_m.replace({c: 'unknown'}, {c: common_value}, regex=True)
   print(train_data_m.head())

print("Input a level(1-16):")
level = int(input())

tree= id3_Entropy(train_data_m, column_labels[16])
...
for i in range(level+1):
    ...
    if(treelevel<i):
        break
    accuracy = evaluate(tree, test_data_m, column_labels[16],i)
    accuracy2 = evaluate(tree, train_data_m, column_labels[16],i)
    print(str(i)+":"+str(accuracy)+","+str(accuracy2))

tree= id3_Gini(train_data_m, column_labels[16])
treelevel=finddepth(tree)/2
...
tree= id3_ME(train_data_m, column_labels[16])
treelevel=finddepth(tree)/2
print("***********************************")
